...
export function useGetAcademicEntriesByGradeAndTerm(grade: number, term: number) {
  ...
  return useQuery<AcademicEntry[]>({
    ...
    queryFn: async () => {
      if (!actor) return [];
      try {
        return await actor.getAcademicEntriesByGradeAndTerm(BigInt(grade), BigInt(term));
      } catch (err) {
        console.error('Failed to fetch academic entries by grade and term:', err);
        throw err;
      }
    },
...
export function useSaveBoardExamResults() {
  ...
  return useMutation<void, Error, { boardExamTotal: number; maxMarks: number }>({
    mutationFn: async ({ boardExamTotal, maxMarks }) => {
      if (!actor) throw new Error('Backend not available. Please try again.');
      return actor.saveBoardExamResults(BigInt(boardExamTotal), BigInt(maxMarks));
    },
    onSuccess: () => {
...
export function useAddCodingChallenge() {
  ...
  return useMutation<
    CodingChallenge,
    Error,
    { title: string; description: string; sampleInput: string; sampleOutput: string }
  >({
    mutationFn: async ({ title, description, sampleInput, sampleOutput }) => {
      if (!actor) throw new Error('Backend not available. Please try again.');
      return actor.addCodingChallenge(title, description, sampleInput, sampleOutput);
    },
    onSuccess: () => {

...
import type { AcademicEntry, BoardExamResults, CodingAttempt, CodingChallenge, UserProfile, UserRole, Subjects, Score9Scale, ExportTypes, GradeAggregates, GradeAggregatesWithWeighting, AllGradesPercentages } from '../backend';
...
export function useCalculateWeightedPercentages() {
  ...
  return useQuery<GradeAggregatesWithWeighting>({
    ...
    queryFn: async () => {
      if (!actor) return { aggregates: [] };
      return actor.calculateWeightedPercentages();
    },
    enabled: !!actor && !isFetching,
    refetchOnMount: 'always',
    staleTime: 0,
...
export function useGetAllGradePercentages() {
  ...
  return useQuery<AllGradesPercentages>({
    ...
    queryFn: async () => {
      ...
      return actor.getAllGradePercentages();
    },
    enabled: !!actor && !isFetching,
    refetchOnMount: 'always',
    staleTime: 0,
...
export function useAddAcademicEntry() {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['weightedPercentages'] });
      await queryClient.invalidateQueries({ queryKey: ['allGradePercentages'] });
      await queryClient.refetchQueries({ queryKey: ['academicEntries'] });
      await queryClient.refetchQueries({ queryKey: ['gradeAggregates'] });
      await queryClient.refetchQueries({ queryKey: ['weightedPercentages'] });
      await queryClient.refetchQueries({ queryKey: ['allGradePercentages'] });
    },
  });
}

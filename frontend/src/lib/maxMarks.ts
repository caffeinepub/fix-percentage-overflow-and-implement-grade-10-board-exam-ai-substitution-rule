...
export function getMaxMarksConfig(grade: number, isBoardExam: boolean = false): MaxMarksConfig {
  ...
  if (grade >= 1 && grade <= 2) {
    ...
    return { regularSubjectMax: 80, computerMax: 80, aiMax: 0 };
  }
...
/**
 * Get the expected max marks for a specific subject key and grade.
 * Used as a fallback when stored max marks are missing or invalid.
 */
export function getExpectedMaxMarksForSubjectKey(
  ...
  if (subjectKey === 'computer') return config.computerMax;
  if (subjectKey === 'ai') return config.aiMax;
...
export function getSubjectsForGrade(
  ...
  if (grade >= 11) {
    ...
    if (stream === 'Science') {
      ...
      fields.push({ name: 'Business Studies', key: 'businessStudies', maxMarks: max });
      ...
      if (!isBoardExam) {
        if (electiveMath === 'maths') {
          ...
          fields.push({ name: 'Applied Maths', key: 'appliedMaths', maxMarks: max });
        }
      } else {
      ...
      fields.push({ name: 'History & Political Science', key: 'ssc', maxMarks: max });
      fields.push({ name: 'Psychology', key: 'psychology', maxMarks: max });
      fields.push({ name: 'Economics', key: 'economics', maxMarks: max });
    }

    // PE as elective for grades 11–12
    fields.push({ name: 'Physical Education (Elective)', key: 'pe', maxMarks: max });

...
export default function UserManagement() {
  ...
  const handleAssign = async () => {
    ...
    try {
      await assignRole.mutateAsync({ user: principalId.trim(), role: selectedRole });
      setSuccessMessage(`Successfully assigned role "${selectedRole}" to ${principalId.trim()}`);
      setPrincipalId('');
    } catch (err) {
        ...
        <CardContent className="space-y-4">
          <Alert>
            ...
            <AlertDescription className="text-sm">
              To find a user's Principal ID, ask them to log in and check their profile page.
              The Principal ID is a unique identifier for each Internet Identity user.
            </AlertDescription>
          </Alert>

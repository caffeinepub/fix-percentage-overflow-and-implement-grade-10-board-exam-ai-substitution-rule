...
import { Subjects, SaveAcademicInput } from '../backend';
...
import { Loader2, BookOpen, Calculator, AlertCircle, Info, Award, FlaskConical, ShoppingBag } from 'lucide-react';
import {
  getSubjectsForGrade,
  isBoardExamGrade,
...
const SCIENCE_SUBGROUPS: { value: ScienceSubgroup; label: string; subjects: string }[] = [
  { value: 'PCM-Psych', label: 'PCM-Psych', subjects: 'Physics, Chemistry, Maths, Psychology' },
  { value: 'PCMCs', label: 'PCMCs', subjects: 'Physics, Chemistry, Maths, Computer Science' },
  { value: 'PCMB', label: 'PCMB', subjects: 'Physics, Chemistry, Maths, Biology' },
];

const COMMERCE_SUBGROUPS: { value: CommerceSubgroup; label: string; subjects: string }[] = [
  { value: 'CEBA', label: 'CEBA', subjects: 'Computer Science, Economics, Business Studies, Accountancy' },
  { value: 'SEBA', label: 'SEBA', subjects: 'Statistics, Economics, Business Studies, Accountancy' },
];

function getScienceSubgroupSubjects(subgroup: ScienceSubgroup, maxMarks: number) {
  const base = [
...
export default function AddMarksForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const [commerceSubgroup, setCommerceSubgroup] = useState<CommerceSubgroup>(null);
  const [marks, setMarks] = useState<Record<string, string>>({});
  const [isBoardExam, setIsBoardExam] = useState(false);
  const [electiveMath, setElectiveMath] = useState<ElectiveMath>(null);
  const [boardExamMathChoice, setBoardExamMathChoice] = useState<'maths' | 'appliedMaths'>('maths');
  ...
  const showElectiveMathSelector = isHighSchool && isCommerceStream && !isBoardExam;
  const showBoardExamMathChoice = isBoardExam && selectedGrade === 12 && isCommerceStream;
  ...
  const getSubjects = () => {
    ...
    if (isHighSchool && isCommerceStream) {
      ...
      return getCommerceSubgroupSubjects(commerceSubgroup, maxMarks, effectiveElectiveMath, isBoardExam);
    }
  ...
  useEffect(() => {
    setMarks({});
    setErrors({});
  }, [selectedGrade, selectedStream, isBoardExam, electiveMath, boardExamMathChoice, scienceSubgroup, commerceSubgroup]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const computerMaxMarks = subjects.find(s => s.key === 'computer')?.maxMarks ?? 0;
    const aiMaxMarks = subjects.find(s => s.key === 'ai')?.maxMarks ?? 0;
    const mathsMaxMarks = subjects.find(s => s.key === 'maths')?.maxMarks ?? 0;
    const appliedMathsMaxMarks = subjects.find(s => s.key === 'appliedMaths')?.maxMarks ?? 0;
    ...
    try {
      ...
      setErrors({ _form: err?.message || 'Failed to save marks. Please try again.' });
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {showBoardExamOption && (
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="board-exam" className="cursor-pointer flex items-center gap-1">
              ...
              <div className="flex flex-wrap gap-2">
                {SCIENCE_SUBGROUPS.map(sg => (
                  ...
                  >
                    ...
                    <span className="text-xs ml-1 opacity-70">({sg.subjects})</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {showBoardExamMathChoice && (
            <div className="space-y-2">
              ...
              <RadioGroup
                ...
                onValueChange={(v) => setBoardExamMathChoice(v as 'maths' | 'appliedMaths')}
                className="flex gap-4"
              >
                <div className="flex items-center gap-2">
          ...
          {isGrade10BoardExam && aiPreview && (
            <Alert>
              ...
              <AlertDescription className="text-sm">
                <strong>AI Substitution:</strong> AI marks ({Number(marks['ai'] || 0)}) will substitute{' '}
                <strong>{aiPreview.substitutedSubject}</strong> if it improves your score.
              </AlertDescription>
            </Alert>
          )}
                ...
                {subjects.map(subject => (
                  <div key={subject.key} className="space-y-1">
                    <Label htmlFor={subject.key} className="text-sm">
                      {subject.name}
                      <span className="text-muted-foreground ml-1 text-xs">/ {subject.maxMarks}</span>
                    </Label>
                    <Input
                      id={subject.key}
                      type="number"
                      ...
                      className={errors[subject.key] ? 'border-destructive' : ''}
                    />
                    {errors[subject.key] && (
                      <p className="text-xs text-destructive">{errors[subject.key]}</p>
                    )}
                  </div>
                ))}
              </div>

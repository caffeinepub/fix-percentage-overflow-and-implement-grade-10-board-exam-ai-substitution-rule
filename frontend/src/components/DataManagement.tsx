...
import type { ExportTypes } from '../backend';
...
export default function DataManagement() {
  ...
  const exportAsCSV = (data: ExportTypes) => {
    ...
    if (exportType === 'academic-entries' || exportType === 'full-export') {
      ...
      csvContent += 'Date,Grade,Term,Stream,Subgroup,Math,English,Hindi,EVS,Computer,Kannada,Science,SSC,AI,Physics,Chemistry,Biology,Economics,Business Studies,Accountancy,Statistics,Management,PE,Total,Max Marks,Percentage\n';
      ...
      for (const [, entries] of data.academicEntries.academicEntries) {
        for (const entry of entries) {
          ...
          csvContent += `${subjects.computer || ''},${subjects.kannada || ''},${subjects.science || ''},${subjects.ssc || ''},`;
          ...
          csvContent += `${subjects.statistics || ''},${subjects.management || ''},${subjects.pe || ''},`;
      ...
      if (data.academicEntries.boardExamResults.length > 0) {
        ...
        for (const [, results] of data.academicEntries.boardExamResults) {
    ...
    if (exportType === 'coding-attempts' || exportType === 'full-export') {
      ...
      for (const [, attempts] of data.coding.attempts) {

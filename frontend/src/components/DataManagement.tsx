...
function convertBigIntsForImport(obj: unknown): unknown {
  ...
  if (typeof obj === 'object') {
    ...
    for (const [key, val] of Object.entries(obj as Record<string, unknown>)) {
      ...
      const stringFields = [
        'stream', 'gradeText', 'scienceSubgroup', 'commerceSubgroup',
        'name', 'title', 'description', 'sampleInput', 'sampleOutput',
        'code', 'result',
      ];
...
function entriesToCSV(entries: AcademicEntry[]): string {
  ...
  const headers = [
    'grade', 'term', 'stream', 'scienceSubgroup', 'commerceSubgroup',
    'termTotalMarks', 'termMaxMarks', 'termPercentage', 'gradeText', 'timestamp',
    'math', 'english', 'hindi', 'evs', 'computer', 'kannada', 'science', 'ssc', 'ai',
    'physics', 'chemistry', 'biology', 'economics', 'businessStudies', 'accountancy',
    'statistics', 'management', 'psychology', 'pe', 'maths', 'appliedMaths',
  ];
  const rows = entries.map((e) => [
    ...
    e.subjects.businessStudies !== undefined ? Number(e.subjects.businessStudies) : '',
    ...
    e.subjects.appliedMaths !== undefined ? Number(e.subjects.appliedMaths) : '',
  ]);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Download your academic and coding data</CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Import previously exported JSON data. This will overwrite existing data.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {importError && (

...
function getStreamDisplay(entry: AcademicEntry): string {
  const stream = entry.stream ?? '';
  const subgroup = getSubgroupLabel(entry);
  if (subgroup) return `${stream} (${subgroup})`;
  return stream;
}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-4">
            ...
            <p className="text-2xl font-bold text-green-600 dark:text-green-400">{stats.highest}%</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
            <p className="text-xs text-muted-foreground">Lowest</p>
            <p className="text-2xl font-bold text-orange-500">{stats.lowest}%</p>
          </CardContent>
        </Card>
      </div>

...
function getSubjectNineScale(
  entry: AcademicEntry,
  subjectKey: string
): { marks: number; maxMarks: number; percentage: number; scale: number } | null {
  ...
  let maxMarks = getExpectedMaxMarksForSubjectKey(subjectKey, grade, term);
...
export default function NineScaleGradesView() {
  ...
  const filtered = entries.filter(e => {
    if (filterGrade !== 'all' && Number(e.grade) !== Number(filterGrade)) return false;
    if (filterTerm !== 'all' && Number(e.term) !== Number(filterTerm)) return false;
    return true;
  });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        {[
          ...
          <div key={score} className="flex items-center gap-1">
            <Badge className={`text-xs px-1.5 py-0.5 ${getNineScaleGradeColor(score)}`}>{score}</Badge>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p className="text-sm mt-1">Add academic entries to see nine-scale grades.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {Array.from(grouped.entries())
            ...
            .map(([grade, termMap]) => (
              <div key={grade}>
                <h3 className="text-lg font-semibold mb-3 text-foreground">Grade {grade}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Array.from(termMap.entries())
                    .sort(([a], [b]) => a - b)
                    .map(([term, entry]) => {
                      const subjectResults = ALL_SUBJECT_KEYS
                        .map(key => ({ key, result: getSubjectNineScale(entry, key) }))
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span className="text-xs text-muted-foreground">
                                      {result!.marks}/{result!.maxMarks} ({result!.percentage.toFixed(1)}%)
                                    </span>
                                    <Badge

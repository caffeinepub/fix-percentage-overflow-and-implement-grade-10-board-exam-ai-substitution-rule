...
function getCertTier(pct: number): { label: string; color: string } | null {
  if (pct >= 95)
    return { label: "Platinum", color: "text-cyan-600 dark:text-cyan-400" };
  if (pct >= 90)
    return { label: "Gold", color: "text-yellow-600 dark:text-yellow-400" };
  if (pct >= 80)
    return { label: "Silver", color: "text-slate-500 dark:text-slate-400" };
  if (pct >= 70)
    return { label: "Bronze", color: "text-amber-700 dark:text-amber-500" };
  return null;
}
    ...
    return (
      <div className="text-center py-12 text-muted-foreground">
        ...
        <p className="text-sm mt-1">
          Add academic entries to see combined percentages.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-muted-foreground">
        Combined % per term = (94% + original term %) ÷ 2, where original term %
        is from your entered marks (same as Progress tab). Overall grade % =
        average of all combined term percentages.
      </p>
      ...
      {Array.from(gradeMap.entries())
        ...
        .map(([grade, gradeEntries]) => {
          ...
          const termCombinedPcts: {
            term: number;
            originalPct: number;
            combinedPct: number;
          }[] = [];
          for (const [term, entry] of Array.from(termMap.entries()).sort(
            ([a], [b]) => a - b,
          )) {
          ...
          const overallScale =
            calculateNineScaleFromPercentage(overallGradePct);
            ...
            <Card key={grade} className="border shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    {overallTier && (
                      <span
                        className={`text-sm font-semibold ${overallTier.color}`}
                      >
                        🏅 {overallTier.label} Certificate
                      </span>
                    )}
                    <Badge
                      className={`${getNineScaleGradeColor(overallScale)} text-sm px-2`}
                    >
                      Scale: {overallScale}
                    </Badge>
                  </div>
              ...
              <CardContent>
                <div className="space-y-3">
                  {/* Per-term breakdown */}
                  {termCombinedPcts.map(
                    ({ term, originalPct, combinedPct }) => {
                      const termScale =
                        calculateNineScaleFromPercentage(combinedPct);
                        ...
                        >
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-muted-foreground">
                              Original: {formatPercent(originalPct, 0)} →
                              Combined: (94 + {originalPct.toFixed(0)}) ÷ 2
                            </span>
                          </div>
                          <div className="flex items-center gap-3">
                            ...
                            {termTier && (
                              <span
                                className={`text-xs font-semibold ${termTier.color}`}
                              >
                                {termTier.label}
                              </span>
                            )}
                            <Badge
                              className={`text-xs ${getNineScaleGradeColor(termScale)}`}
                            >
                              {termScale}
                            </Badge>
                          </div>
                  ...
                  <div className="flex items-center justify-between pt-2 mt-1 border-t-2 border-primary/30">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-muted-foreground">
                        Sum of combined term % ÷ {termCount} term
                        {termCount !== 1 ? "s" : ""}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <Badge
                        className={`text-sm font-bold ${getNineScaleGradeColor(overallScale)}`}
                      >
                        {overallScale}
                      </Badge>
                    </div>

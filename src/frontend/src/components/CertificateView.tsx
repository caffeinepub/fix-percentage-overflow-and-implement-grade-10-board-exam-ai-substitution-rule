...
function getCertTier(combinedPct: number): CertTier | null {
  if (combinedPct >= 95)
    return {
      ...
      bgClass:
        "bg-gradient-to-br from-cyan-50 to-slate-100 dark:from-cyan-950 dark:to-slate-900",
  ...
  if (combinedPct >= 90)
    return {
      ...
      bgClass:
        "bg-gradient-to-br from-yellow-50 to-amber-100 dark:from-yellow-950 dark:to-amber-900",
...
function CertificateCard({
  ...
  return (
    <Card
      className={`border-2 ${tier.borderClass} ${tier.bgClass} overflow-hidden shadow-md`}
    >
          ...
          <div className="border-t border-current/10 pt-2 space-y-1">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-muted-foreground">
                {formatPercent(originalPct, 0)}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">
                Combined % <span className="text-xs">(94+original)÷2</span>
              </span>
...
export default function CertificateView() {
  ...
  if (entriesError)
    return <ErrorMessage message="Failed to load academic entries." />;
    ...
    return (
      <div className="text-center py-12 text-muted-foreground">
        ...
        <p className="text-sm mt-1">
          Add academic entries to earn certificates.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="space-y-1">
        <p className="text-sm text-muted-foreground">
          Certificate tier is based on combined % = (94 + original term %) ÷ 2,
          where original term % comes from your entered marks (same as Progress
          tab).
        </p>
        <p className="text-xs text-muted-foreground">
          💎 Platinum: 95–100% &nbsp;|&nbsp; 🥇 Gold: 90–94% &nbsp;|&nbsp; 🥈
          Silver: 80–89% &nbsp;|&nbsp; 🥉 Bronze: 70–79%
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="flex items-center gap-2">
          <label htmlFor="cert-grade-filter" className="text-sm font-medium">
            Grade:
          </label>
          <Select
            value={filterGrade}
          ...
          >
            <SelectTrigger id="cert-grade-filter" className="w-32">
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="cert-term-filter" className="text-sm font-medium">
            Term:
          </label>
          <Select value={filterTerm} onValueChange={setFilterTerm}>
            <SelectTrigger id="cert-term-filter" className="w-28">
